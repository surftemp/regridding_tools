...
class TimeAggregator(object):
    ...
    def __init__(self,lon_min,lat_min,lon_max,lat_max,f_max=1.0,output_anomaly=False,output_sea_ice=False,
    ...
    def aggregate(self,start_dt,end_dt,data):
        ...
        mean_sea_water_temp_numerator = (sea_water_temp*self.lat_weighting*sea_ice_mask*land_mask).sum(**axis_params)
        ...
        if self.output_anomaly:
            ...
            mean_sea_water_climatology_numerator = (climatology*self.lat_weighting*sea_ice_mask*land_mask).sum(**axis_params)
        ...
        n = np.minimum(k,N)
        uncertainty_numerator = (standard_squared_error*self.lat_weighting*sea_ice_mask*land_mask).sum(**axis_params)
...
class RegionExtractor(object):
    ...
    def generateYearData(self,start_date,end_date,time_resolution,min_lon,min_lat,max_lon,max_lat):
        ...
        time_periods = RegionExtractor.createTimePeriods(time_resolution,start_date,end_date)
...
def makeRegionSSTs(lon_min,lon_max,lat_min,lat_max,time_resolution,
    ...
    extractor = RegionExtractor(input_path)

    # create an aggregator to aggregate each period in the extracted data
    aggregator = TimeAggregator(lon_min=lon_min, lat_min=lat_min, lon_max=lon_max,
